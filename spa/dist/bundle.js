...
},{"./collection":3,"./request":4,"./router":5,"./sections":6}],3:[function(require,module,exports){
'use strict';
	...
	_createClass(Collection, [{
		...
		value: function requestArtwork(artwork) {
			...
			this.app.request.get(request).then(function (response) {
				...
				return result.artObjects ? _this.renderCollection(result.artObjects) : _this.renderArtwork(result.artObject);
			}).catch(function (err) {
		...
		value: function renderCollection(collection) {
			...
			section.insertAdjacentHTML('beforeend', collection.reduce(function (allArt, artwork) {
		...
		value: function renderArtwork(artwork) {
			var article = document.querySelector('#' + artwork.objectNumber);
